...
async def _execute_marketing_plan(plan: dict[str, Any]) -> Any:
    ...
    if api == "direct":
        ...
        if response.status_code >= 400:
            raise RuntimeError(
                f"Yandex Direct returned HTTP {response.status_code}: {_safe_response_text(response)}"
            )
        return response.json()
...
@mcp.tool(
    ...
    description=(
        ...
        "Yandex Metrica API relative path. Does not send the write request."
    ),
...
@mcp.tool(
    ...
    description=(
        ...
        "plan. The request is not sent until apply_confirmed_marketing_write is called "
        "with this exact plan and one-time confirmation token after explicit user approval."
    ),
    annotations=ToolAnnotations(readOnlyHint=False, destructiveHint=False, idempotentHint=False, openWorldHint=True),
)
...
@mcp.tool(
    title="Apply confirmed Yandex Marketing write",
    description=(
        "WRITE/DESTRUCTIVE. Execute exactly one immutable Metrica or Direct plan previously "
        "created by a preview tool. Requires the exact one-time confirmation token. "
        "Call only after the user explicitly confirms that exact preview."
    ),
    annotations=ToolAnnotations(readOnlyHint=False, destructiveHint=True, idempotentHint=False, openWorldHint=True),
)
async def apply_confirmed_marketing_write(
    plan_id: str,
    ...
    # Consume before the network call to prevent accidental duplicate mutation.
    plan["used"] = True
    try:
