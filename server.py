...
async def marketing_api_status() -> dict[str, Any]:
    return {
        "ok": True,
        "server_revision": "v5.2-fixed-per-group-actions",
...
async def _rostov_prepare_operation(operation: str) -> dict[str, Any]:
    ...
    if group_name:
        ...
        if len(autos) != 1 or str(autos[0].get("State") or "") != "SUSPENDED":
            return {
                "ok": False,
                "blocked_by_precondition": True,
                "reason": "group_autotargeting_must_be_suspended_first",
                "group": group_name,
            }
        items = [
    ...
    if operation == "sync_regions_from_group_06_v1":
        ...
        if not source_regions:
            return {"ok": False, "blocked_by_precondition": True, "reason": "source_group_06_has_no_regions"}
    ...
    if operation == "add_keywords_v1":
        ...
        if len(autotargetings) != 6:
            return {
                "ok": False,
                "blocked_by_precondition": True,
                "reason": "expected_six_autotargetings_before_manual_keywords",
                "found": len(autotargetings),
            }
        active_autotargetings = [
            k for k in autotargetings if str(k.get("State") or "") != "SUSPENDED"
        ]
        if active_autotargetings:
            return {
                "ok": False,
                "blocked_by_precondition": True,
                "reason": "autotargeting_must_be_suspended_first",
                "ids": [k.get("Id") for k in active_autotargetings],
                "states": [k.get("State") for k in active_autotargetings],
            }
...
async def _rostov_verify_operation(operation: str) -> dict[str, Any]:
    ...
    if operation == "sync_regions_from_group_06_v1":
        ...
        source_regions = list(groups["06 Конфигурация"].get("RegionIds") or [])
        mismatched = [n for n in ROSTOV_KITCHEN_GROUP_NAMES if list(groups[n].get("RegionIds") or []) != source_regions]
        return {"ok": bool(source_regions) and not mismatched, "region_ids": source_regions, "mismatched": mismatched}
    ...
    if operation == "add_keywords_v1":
        ...
        active_autotargeting = [k for k in autotargeting if str(k.get("State") or "") != "SUSPENDED"]
        return {
            "ok": not missing and len(autotargeting) == 6 and not active_autotargeting,
            "missing": missing,
