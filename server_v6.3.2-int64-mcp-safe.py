...
DIRECT_V501_BASE = "https://api.direct.yandex.com/json/v501"
DIRECT_REPORTS_URL = "https://api.direct.yandex.com/json/v5/reports"
DIRECT_V501_SERVICES = {"ads", "adimages", "sitelinks", "adextensions"}
...
async def _execute_marketing_plan(plan: dict[str, Any]) -> Any:
    ...
    if api == "direct":
        ...
        async with httpx.AsyncClient(timeout=REQUEST_TIMEOUT, follow_redirects=True) as client:
            response = await client.post(_direct_service_url(service), headers=headers, json=body)
        meta = _direct_response_meta(response)
        response_text = _safe_response_text(response, limit=12000)
        logger.info(
...
async def marketing_api_status() -> dict[str, Any]:
    return {
        "ok": True,
        "server_revision": "v6.3.2-int64-mcp-safe",
        ...
        "write_policy": "allow-listed generic safe writes + legacy fixed operations -> preview -> immutable plan -> explicit confirmation -> apply -> read-back verification",
        ...
        "legacy_direct_apply_tool": "apply_rostov_kitchen_launch_operation",
        "direct_write_capabilities": [
            "generic_safe_update_campaign_adgroup_keyword_textad_responsivead",
            "generic_safe_actions_suspend_resume_archive_unarchive_moderate",
            "responsive_ad_titles_texts_images_sitelinks_callouts",
            "safe_add_sitelink_sets_callouts_ad_images",
            "ads_v501_write_and_sitelinks_verification_fix",
            "generic_safe_add_textcampaign_adgroup_keyword_textad_responsivead",
...
def _direct_service_url(service: str) -> str:
    service_name = str(service or "").strip().lower()
    base = DIRECT_V501_BASE if service_name in DIRECT_V501_SERVICES else DIRECT_BASE
    return f"{base}/{service_name}"


DIRECT_MCP_JS_SAFE_INTEGER = 9_007_199_254_740_991


def _direct_mcp_lossless_ints(value: Any) -> Any:
    """Return MCP-safe data without losing Direct int64 IDs in JavaScript clients.

    Internal Direct requests and immutable plans keep Python integers, so Yandex receives
    the exact numeric ID. Only values crossing the MCP response boundary are normalized:
    integers outside JavaScript's safe range are emitted as decimal strings.
    """
    if value is None or isinstance(value, bool):
        return value
    if isinstance(value, int):
        if abs(value) > DIRECT_MCP_JS_SAFE_INTEGER:
            return str(value)
        return value
    if isinstance(value, dict):
        return {k: _direct_mcp_lossless_ints(v) for k, v in value.items()}
    if isinstance(value, (list, tuple)):
        return [_direct_mcp_lossless_ints(v) for v in value]
    return value
...
@mcp.tool(
    ...
    description=(
        "READ-ONLY universal Direct API reader. Uses v501 for ads/assets services that "
        "require it and v5 for the remaining services; only get is allowed here."
    ),
    annotations=ToolAnnotations(readOnlyHint=True, destructiveHint=False, idempotentHint=True, openWorldHint=True),
)
async def direct_get(
    ...
    async with httpx.AsyncClient(timeout=REQUEST_TIMEOUT, follow_redirects=True) as client:
        response = await client.post(_direct_service_url(service_name), headers=headers, json=body)
    return _direct_mcp_lossless_ints(_parse_direct_json(response, operation=f"{service_name}.get"))
...
async def _direct_request(service: str, body: dict[str, Any]) -> httpx.Response:
    ...
    async with httpx.AsyncClient(timeout=REQUEST_TIMEOUT, follow_redirects=True) as client:
        return await client.post(
            _direct_service_url(service_name),
            headers=headers,
            json=body,
        )
...
DIRECT_SAFE_ENTITY_ALIASES = {
    ...
    "combinatorial_ad": "responsive_ad",
    "ad": "text_ad",
    "ads": "text_ad",
}
DIRECT_SAFE_ACTIONS = {
    ...
    "responsive_ad": {"suspend", "resume", "moderate", "archive", "unarchive"},
}
...
def _direct_safe_entity(value: Any) -> str:
    ...
    if not entity:
        raise ValueError(
            "Unsupported Direct safe-write entity. Allowed: campaign, ad_group, keyword, text_ad, responsive_ad."
        )
    return entity


def _direct_safe_positive_id(value: Any, name: str = "id") -> int:
    """Parse a Direct long ID losslessly. Public MCP tools accept decimal strings to avoid JS rounding."""
    if isinstance(value, bool):
        raise ValueError(f"{name} must be a positive integer or decimal string.")
    if isinstance(value, float):
        raise ValueError(f"{name} must not be a floating-point number; use a decimal string for long IDs.")
    text = str(value or "").strip()
    if not re.fullmatch(r"[1-9]\d{0,18}", text):
        raise ValueError(f"{name} must be a positive decimal integer with at most 19 digits.")
    parsed = int(text)
    if parsed <= 0:
...
def _direct_safe_id_text(value: Any, name: str = "id") -> str:
    return str(_direct_safe_positive_id(value, name))
...
async def _direct_safe_get_entity(entity: str, entity_id: Any) -> Optional[dict[str, Any]]:
    entity = _direct_safe_entity(entity)
    entity_id_int = _direct_safe_positive_id(entity_id)
    if entity == "campaign":
        body = {
            "method": "get",
            "params": {
                "SelectionCriteria": {"Ids": [entity_id_int]},
                "FieldNames": [
        ...
        body = {
            ...
            "params": {
                ...
                "TextAdFieldNames": [
                    ...
                    "SitelinkSetId", "AdExtensions", "AdImageHash"
                ],
                "ResponsiveAdFieldNames": [
                    "Titles", "Texts", "Href", "DisplayUrlPath", "SitelinkSetId",
                    "AdImages", "AdExtensions", "BusinessId"
                ],
            },
        }
    ...
    if obj and entity == "responsive_ad" and str(obj.get("Type") or "") != "RESPONSIVE_AD":
        return None
    if obj and entity == "text_ad" and str(obj.get("Type") or "") not in {"TEXT_AD", "RESPONSIVE_AD"}:
        return None
    return obj
...
def _direct_safe_snapshot(entity: str, obj: dict[str, Any], change_keys: list[str]) -> dict[str, Any]:
    ...
    elif entity == "responsive_ad":
        ra = obj.get("ResponsiveAd") or {}
        title_items = ra.get("Titles") or []
        text_items = ra.get("Texts") or []
        image_items = ((ra.get("AdImages") or {}).get("Items") or [])
        extension_items = ra.get("AdExtensions") or []
        for key in change_keys:
            if key == "titles":
                snapshot[key] = [str(x.get("Title") or "") for x in title_items if isinstance(x, dict)]
            elif key == "texts":
                snapshot[key] = [str(x.get("Text") or "") for x in text_items if isinstance(x, dict)]
            elif key == "href":
                snapshot[key] = ra.get("Href")
            elif key == "display_url_path":
                snapshot[key] = ra.get("DisplayUrlPath")
            elif key == "sitelink_set_id":
                snapshot[key] = ra.get("SitelinkSetId")
            elif key == "ad_image_hashes":
                snapshot[key] = [str(x.get("ImageHash") or "") for x in image_items if isinstance(x, dict)]
            elif key == "ad_extension_ids":
                snapshot[key] = [int(x.get("AdExtensionId")) for x in extension_items if isinstance(x, dict) and x.get("AdExtensionId")]
    else:
        # TextAd remains supported for legacy objects. If Yandex has already auto-upgraded
        # the object to RESPONSIVE_AD, use entity=responsive_ad instead of silently coercing it.
...
def _direct_safe_compile_text_ad_update(
    ...
    if str(ad.get("Type") or "") != "TEXT_AD":
        raise ValueError("This object is not TEXT_AD. Use entity=responsive_ad for RESPONSIVE_AD objects.")
...
def _direct_safe_validate_word_lengths(text: str, *, max_word: int, name: str) -> None:
    words = [w.strip(".,!?;:\"'()[]{}") for w in re.split(r"\s+", text) if w.strip()]
    too_long = [w for w in words if len(w) > max_word]
    if too_long:
        raise ValueError(f"{name} contains a word longer than {max_word} characters: {too_long[0]!r}")


def _direct_safe_responsive_strings(value: Any, name: str, *, min_items: int, max_items: int, max_length: int, max_word: int) -> list[str]:
    if not isinstance(value, list):
        raise ValueError(f"{name} must be a list.")
    if not (min_items <= len(value) <= max_items):
        raise ValueError(f"{name} must contain {min_items}..{max_items} items.")
    result: list[str] = []
    seen: set[str] = set()
    for i, raw in enumerate(value):
        text = _direct_safe_clean_string(raw, f"{name}[{i}]", max_length=max_length)
        _direct_safe_validate_word_lengths(text, max_word=max_word, name=f"{name}[{i}]")
        key = text.casefold()
        if key in seen:
            raise ValueError(f"{name} must contain distinct variants; duplicate: {text!r}")
        seen.add(key)
        result.append(text)
    return result


def _direct_safe_hashes(value: Any, name: str = "ad_image_hashes") -> list[str]:
    vals = _direct_safe_clean_string_list(value, name, max_items=5, max_item_length=128, allow_empty=True)
    for h in vals:
        if not re.fullmatch(r"[A-Za-z0-9_-]{8,128}", h):
            raise ValueError(f"{name} contains an invalid Direct image hash.")
    return vals


def _direct_safe_id_list(value: Any, name: str, *, max_items: int) -> list[int]:
    if not isinstance(value, list):
        raise ValueError(f"{name} must be a list.")
    if len(value) > max_items:
        raise ValueError(f"{name} supports at most {max_items} items.")
    result: list[int] = []
    for raw in value:
        v = _direct_safe_positive_id(raw, name)
        if v not in result:
            result.append(v)
    return result


def _direct_safe_compile_responsive_ad_update(
    ad: dict[str, Any], changes: dict[str, Any]
) -> tuple[dict[str, Any], list[str], dict[str, Any]]:
    if str(ad.get("Type") or "") != "RESPONSIVE_AD":
        raise ValueError("Generic responsive_ad update requires a RESPONSIVE_AD object.")
    allowed = {
        "titles", "texts", "href", "display_url_path", "sitelink_set_id",
        "ad_image_hashes", "ad_extension_ids",
    }
    unknown = set(changes) - allowed
    if unknown:
        raise ValueError(f"Unsupported responsive_ad update fields: {sorted(unknown)}")

    current = ad.get("ResponsiveAd") or {}
    current_titles = [str(x.get("Title") or "") for x in (current.get("Titles") or []) if isinstance(x, dict)]
    current_texts = [str(x.get("Text") or "") for x in (current.get("Texts") or []) if isinstance(x, dict)]
    if not current_titles or not current_texts:
        raise ValueError("Responsive ad read-back is missing current titles/texts; refusing unsafe partial update.")

    ra: dict[str, Any] = {"Titles": current_titles, "Texts": current_texts}
    if current.get("Href"):
        ra["Href"] = current.get("Href")
    elif current.get("BusinessId"):
        ra["BusinessId"] = int(current.get("BusinessId"))

    change_keys: list[str] = []
    expected: dict[str, Any] = {}
    if "titles" in changes:
        titles = _direct_safe_responsive_strings(changes["titles"], "titles", min_items=1, max_items=7, max_length=56, max_word=22)
        ra["Titles"] = titles; change_keys.append("titles"); expected["titles"] = titles
    if "texts" in changes:
        texts = _direct_safe_responsive_strings(changes["texts"], "texts", min_items=1, max_items=3, max_length=81, max_word=23)
        ra["Texts"] = texts; change_keys.append("texts"); expected["texts"] = texts
    if "href" in changes:
        raw = changes["href"]
        href = None if raw is None else _direct_safe_clean_string(raw, "href", max_length=1024)
        if href and not re.match(r"^https?://", href, flags=re.I):
            raise ValueError("href must start with http:// or https://")
        ra["Href"] = href; change_keys.append("href"); expected["href"] = href
    if "display_url_path" in changes:
        raw = changes["display_url_path"]
        path = None if raw is None else _direct_safe_clean_string(raw, "display_url_path", max_length=20, allow_empty=True)
        if path is not None and (" " in path or "_" in path or "--" in path or "//" in path):
            raise ValueError("display_url_path contains a forbidden sequence.")
        ra["DisplayUrlPath"] = path; change_keys.append("display_url_path"); expected["display_url_path"] = path
    if "sitelink_set_id" in changes:
        raw = changes["sitelink_set_id"]
        value = None if raw is None else _direct_safe_positive_id(raw, "sitelink_set_id")
        ra["SitelinkSetId"] = value; change_keys.append("sitelink_set_id"); expected["sitelink_set_id"] = value
    if "ad_image_hashes" in changes:
        raw = changes["ad_image_hashes"]
        hashes = [] if raw is None else _direct_safe_hashes(raw)
        ra["AdImageHashes"] = {"Items": hashes} if hashes else None
        change_keys.append("ad_image_hashes"); expected["ad_image_hashes"] = hashes
    if "ad_extension_ids" in changes:
        raw = changes["ad_extension_ids"]
        ids = [] if raw is None else _direct_safe_id_list(raw, "ad_extension_ids", max_items=50)
        ra["CalloutSetting"] = None if not ids else {
            "AdExtensions": [{"AdExtensionId": i, "Operation": "SET"} for i in ids]
        }
        change_keys.append("ad_extension_ids"); expected["ad_extension_ids"] = ids

    proposed_href = ra.get("Href")
    if ("SitelinkSetId" in ra and ra.get("SitelinkSetId") is not None) and not proposed_href:
        raise ValueError("sitelink_set_id requires a non-empty href on responsive_ad.")
    if not change_keys:
        raise ValueError("No usable responsive_ad changes were supplied.")
    return {"Id": _direct_safe_positive_id(ad["Id"]), "ResponsiveAd": ra}, change_keys, expected
...
async def _direct_safe_preview_update_internal(
    entity: str, entity_id: Any, changes: dict[str, Any]
) -> dict[str, Any]:
    entity = _direct_safe_entity(entity)
    ...
    if entity == "campaign":
        ...
        item, change_keys, expected = _direct_safe_compile_responsive_ad_update(current, changes)
    ...
    preview["summary"] = {
        "operation": "update", "entity": entity, "entity_id": str(entity_id),
...
async def direct_safe_write_capabilities() -> dict[str, Any]:
    return {
        ...
        "generic_add": [
            "text_campaign", "ad_group", "keyword", "text_ad", "responsive_ad",
            "sitelink_set", "callout", "ad_image",
        ],
...
async def preview_direct_safe_update(
    ...
    preview = await _direct_safe_preview_update_internal(entity, entity_id, changes)
    return _direct_mcp_lossless_ints(preview)
...
async def preview_direct_safe_action(
    ...
    for raw in entity_ids:
        ...
        id_texts.append(str(entity_id))
        before[str(entity_id)] = await _direct_safe_state_snapshot(entity, obj)
    service = {"campaign": "campaigns", "keyword": "keywords", "text_ad": "ads", "responsive_ad": "ads"}[entity]
    ...
    preview = _make_marketing_plan("direct", "POST", service, params=metadata, payload=payload)
    preview["safe_write"] = True
    preview["summary"] = {"operation": "action", "entity": entity, "action": action, "ids": id_texts, "current": before}
    return _direct_mcp_lossless_ints(preview)
...
def _direct_safe_compile_add(entity: str, spec: dict[str, Any]) -> tuple[str, str, dict[str, Any], dict[str, Any]]:
    ...
    if entity == "responsive_ad":
        allowed = {
            "ad_group_id", "titles", "texts", "href", "display_url_path", "sitelink_set_id",
            "ad_image_hashes", "ad_extension_ids",
        }
        unknown = set(spec) - allowed
        if unknown:
            raise ValueError(f"Unsupported responsive_ad add fields: {sorted(unknown)}")
        titles = _direct_safe_responsive_strings(spec.get("titles"), "titles", min_items=1, max_items=7, max_length=56, max_word=22)
        texts = _direct_safe_responsive_strings(spec.get("texts"), "texts", min_items=1, max_items=3, max_length=81, max_word=23)
        ra: dict[str, Any] = {"Titles": titles, "Texts": texts}
        if "href" in spec:
            href = _direct_safe_clean_string(spec["href"], "href", max_length=1024)
            if not re.match(r"^https?://", href, flags=re.I):
                raise ValueError("href must start with http:// or https://")
            ra["Href"] = href
        if "display_url_path" in spec:
            path = _direct_safe_clean_string(spec["display_url_path"], "display_url_path", max_length=20, allow_empty=True)
            if " " in path or "_" in path or "--" in path or "//" in path:
                raise ValueError("display_url_path contains a forbidden sequence.")
            ra["DisplayUrlPath"] = path
        if "sitelink_set_id" in spec:
            if not ra.get("Href"):
                raise ValueError("sitelink_set_id requires href on responsive_ad.")
            ra["SitelinkSetId"] = _direct_safe_positive_id(spec["sitelink_set_id"], "sitelink_set_id")
        if "ad_image_hashes" in spec:
            hashes = _direct_safe_hashes(spec["ad_image_hashes"])
            if hashes:
                ra["AdImageHashes"] = hashes
        if "ad_extension_ids" in spec:
            ids = _direct_safe_id_list(spec["ad_extension_ids"], "ad_extension_ids", max_items=50)
            if ids:
                ra["AdExtensionIds"] = ids
        if not ra.get("Href"):
            raise ValueError("responsive_ad add currently requires href in this safe gateway.")
        ad_group_id = _direct_safe_positive_id(spec.get("ad_group_id"), "ad_group_id")
        item = {"AdGroupId": ad_group_id, "ResponsiveAd": ra}
        expected = {
            "titles": titles, "texts": texts, "ad_group_id": ad_group_id,
            "href": ra.get("Href"),
            "sitelink_set_id": ra.get("SitelinkSetId"),
            "ad_image_hashes": list(ra.get("AdImageHashes") or []),
            "ad_extension_ids": list(ra.get("AdExtensionIds") or []),
        }
        return "ads", "Ads", item, expected
    if entity == "sitelink_set":
        if set(spec) != {"sitelinks"}:
            raise ValueError("sitelink_set add accepts only the sitelinks field.")
        raw_items = spec.get("sitelinks")
        if not isinstance(raw_items, list) or not (1 <= len(raw_items) <= 8):
            raise ValueError("sitelinks must contain 1..8 items.")
        out: list[dict[str, Any]] = []
        for i, raw in enumerate(raw_items):
            if not isinstance(raw, dict):
                raise ValueError(f"sitelinks[{i}] must be an object.")
            unknown = set(raw) - {"title", "href", "description"}
            if unknown:
                raise ValueError(f"sitelinks[{i}] has unsupported fields: {sorted(unknown)}")
            title = _direct_safe_clean_string(raw.get("title"), f"sitelinks[{i}].title", max_length=30)
            href = _direct_safe_clean_string(raw.get("href"), f"sitelinks[{i}].href", max_length=1024)
            if not re.match(r"^https?://", href, flags=re.I):
                raise ValueError(f"sitelinks[{i}].href must start with http:// or https://")
            item_sl: dict[str, Any] = {"Title": title, "Href": href}
            if raw.get("description") is not None:
                item_sl["Description"] = _direct_safe_clean_string(raw.get("description"), f"sitelinks[{i}].description", max_length=60, allow_empty=True)
            out.append(item_sl)
        return "sitelinks", "SitelinksSets", {"Sitelinks": out}, {"sitelinks": out}
    if entity == "callout":
        if set(spec) != {"text"}:
            raise ValueError("callout add accepts only the text field.")
        text_value = _direct_safe_clean_string(spec.get("text"), "text", max_length=25)
        return "adextensions", "AdExtensions", {"Callout": {"CalloutText": text_value}}, {"text": text_value}
    if entity == "ad_image":
        allowed = {"name", "image_base64", "type"}
        unknown = set(spec) - allowed
        if unknown:
            raise ValueError(f"Unsupported ad_image add fields: {sorted(unknown)}")
        name = _direct_safe_clean_string(spec.get("name"), "name", max_length=255)
        data = str(spec.get("image_base64") or "").strip()
        if not data or len(data) > 20_000_000:
            raise ValueError("image_base64 must be non-empty and at most 20,000,000 characters.")
        try:
            decoded = base64.b64decode(data, validate=True)
        except Exception as exc:
            raise ValueError("image_base64 is not strict Base64.") from exc
        if len(decoded) < 100:
            raise ValueError("image_base64 is unexpectedly small.")
        image_type = str(spec.get("type") or "AUTO").strip().upper()
        if image_type not in {"AUTO", "REGULAR", "WIDE", "FIXED_IMAGE"}:
            raise ValueError("ad_image.type must be AUTO, REGULAR, WIDE or FIXED_IMAGE.")
        item = {"ImageData": data, "Type": image_type, "Name": name}
        expected = {"name": name, "type_requested": image_type, "sha256": _sha256_bytes(decoded)}
        return "adimages", "AdImages", item, expected
    raise ValueError("Unsupported add entity. Allowed: text_campaign, ad_group, keyword, text_ad, responsive_ad, sitelink_set, callout, ad_image.")
...
async def _direct_safe_parent_precheck(entity: str, item: dict[str, Any]) -> None:
    ...
    elif entity in {"text_ad", "responsive_ad"}:
        parent = await _direct_safe_get_entity("ad_group", item["AdGroupId"])
        if not parent:
            raise ValueError("Parent ad group is not accessible to this Direct account.")
...
@mcp.tool(
    ...
    description=(
        "PREVIEW ONLY. Creates one allow-listed TextCampaign, ad group, keyword/autotargeting, "
        "TextAd/ResponsiveAd, sitelink set, callout or ad image specification. No raw Direct "
        "service/method/payload is accepted. Parent objects are "
...
async def preview_direct_safe_add(entity: str, spec: dict[str, Any]) -> dict[str, Any]:
    ...
    if normalized_entity == "ad_image":
        # Keep image bytes only in the server-side immutable plan store; never echo Base64 back to the client.
        preview["payload"] = {"method": "add", "params": {"AdImages": [{
            "Name": item["Name"], "Type": item["Type"], "ImageData": "<redacted>"
        }]}}
    return _direct_mcp_lossless_ints(preview)
...
def _direct_safe_validate_plan_shape(plan: dict[str, Any]) -> tuple[str, str, dict[str, Any]]:
    ...
    if operation == "update":
        expected_service = {"campaign": "campaigns", "ad_group": "adgroups", "keyword": "keywords", "text_ad": "ads", "responsive_ad": "ads"}.get(entity)
        list_key = {"campaign": "Campaigns", "ad_group": "AdGroups", "keyword": "Keywords", "text_ad": "Ads", "responsive_ad": "Ads"}.get(entity)
        ...
        if len(arr) != 1 or str(arr[0].get("Id") or "") != str(meta.get("entity_id") or ""):
            raise ValueError("Safe update plan object ID mismatch.")
    elif operation == "action":
        expected_service = {"campaign": "campaigns", "keyword": "keywords", "text_ad": "ads", "responsive_ad": "ads"}.get(entity)
        ...
        if [str(x) for x in payload_ids] != [str(x) for x in (meta.get("entity_ids") or [])]:
            raise ValueError("Safe action plan IDs mismatch.")
    elif operation == "add":
        expected_service = {
            "text_campaign": "campaigns", "ad_group": "adgroups", "keyword": "keywords",
            "text_ad": "ads", "responsive_ad": "ads", "sitelink_set": "sitelinks",
            "callout": "adextensions", "ad_image": "adimages",
        }.get(entity)
        list_key = {
            "text_campaign": "Campaigns", "ad_group": "AdGroups", "keyword": "Keywords",
            "text_ad": "Ads", "responsive_ad": "Ads", "sitelink_set": "SitelinksSets",
            "callout": "AdExtensions", "ad_image": "AdImages",
        }.get(entity)
        ...
        allowed_keys = {
            ...
            "responsive_ad": {"AdGroupId", "ResponsiveAd"},
            "sitelink_set": {"Sitelinks"},
            "callout": {"Callout"},
            "ad_image": {"ImageData", "Type", "Name"},
        }[entity]
...
async def _direct_safe_verify_plan(plan: dict[str, Any], operation: str, entity: str, meta: dict[str, Any], applied: Optional[dict[str, Any]]) -> dict[str, Any]:
    ...
    if len(add_results) != 1:
        return {"verified": False, "reason": "missing_add_result"}
    expected = dict(meta.get("expected_snapshot") or {})

    if entity == "ad_image":
        image_hash = str(add_results[0].get("AdImageHash") or "")
        if not image_hash:
            return {"verified": False, "reason": "missing_ad_image_hash"}
        response = await _direct_request("adimages", {
            "method": "get", "params": {
                "SelectionCriteria": {"AdImageHashes": [image_hash]},
                "FieldNames": ["AdImageHash", "Name", "Type", "Subtype", "Associated"],
            }
        })
        data = _parse_direct_json(response, operation="adimages.get safe-write verification")
        items = ((data.get("result") or {}).get("AdImages") or [])
        actual = {"name": items[0].get("Name")} if items else None
        verified = bool(items and actual.get("name") == expected.get("name"))
        return {"verified": verified, "created_hash": image_hash, "actual": actual, "expected": expected}

    new_id_raw = add_results[0].get("Id")
    if not new_id_raw:
        return {"verified": False, "reason": "missing_add_result_id"}
    new_id = str(new_id_raw)

    if entity == "sitelink_set":
        response = await _direct_request("sitelinks", {
            "method": "get", "params": {
                "SelectionCriteria": {"Ids": [_direct_safe_positive_id(new_id)]},
                # With Sitelinks in FieldNames, Direct already returns
                # Title/Href/Description. SitelinkFieldNames must not be
                # sent together with FieldNames=["...","Sitelinks"].
                "FieldNames": ["Id", "Sitelinks"],
            }
        })
        data = _parse_direct_json(response, operation="sitelinks.get safe-write verification")
        items = ((data.get("result") or {}).get("SitelinksSets") or [])
        actual = {"sitelinks": items[0].get("Sitelinks") or []} if items else None
        return {"verified": bool(items and actual == expected), "created_id": new_id, "actual": actual, "expected": expected}
    if entity == "callout":
        response = await _direct_request("adextensions", {
            "method": "get", "params": {
                "SelectionCriteria": {"Ids": [_direct_safe_positive_id(new_id)]},
        ...
        data = _parse_direct_json(response, operation="adextensions.get safe-write verification")
        items = ((data.get("result") or {}).get("AdExtensions") or [])
        actual = {"text": ((items[0].get("Callout") or {}).get("CalloutText"))} if items else None
        return {"verified": bool(items and actual == expected), "created_id": new_id, "actual": actual, "expected": expected}
    ...
    if entity == "text_campaign":
        ...
        keys = ["titles", "texts", "href", "sitelink_set_id", "ad_image_hashes", "ad_extension_ids"]
        actual = _direct_safe_snapshot("responsive_ad", obj, keys)
        actual["ad_group_id"] = obj.get("AdGroupId")
    else:
        actual = {
...
async def apply_direct_safe_write(
    ...
    try:
        ...
        if verification.get("verified"):
            ...
            return _direct_mcp_lossless_ints({
                "ok": True,
                "applied": True,
...
# Legacy fixed campaign operation retained for backward compatibility. Generic writes are available only through the allow-listed Direct Safe Write Framework.
