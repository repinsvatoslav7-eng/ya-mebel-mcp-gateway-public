...
async def marketing_api_status() -> dict[str, Any]:
    return {
        "ok": True,
        "server_revision": "v5.0-rostov-launch-pack",
...
async def _rostov_prepare_operation(operation: str) -> dict[str, Any]:
    ...
    if operation == "add_keywords_v1":
        ...
        existing_target = [k for k in existing if int(k.get("AdGroupId") or 0) in target_gids]
        if existing_target:
            return {
                "ok": False,
                "blocked_by_precondition": True,
                "reason": "target_groups_already_have_keywords_or_autotargeting",
...
async def _rostov_verify_operation(operation: str) -> dict[str, Any]:
    ...
    if operation == "add_keywords_v1":
        ...
        return {
            "ok": not missing and not autotargeting,
            "missing": missing,
...
@mcp.tool(
    ...
    description=(
        ...
        "names are create_groups_v1, set_campaign_negatives_v1, add_keywords_v1, "
