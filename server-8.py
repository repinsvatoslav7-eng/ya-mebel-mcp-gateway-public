...
class ManagedStyleRule(BaseModel):
    """One declarative page-scoped CSS rule; no PHP, JavaScript, HTML, URLs, or raw stylesheet blocks."""
    rule_id: Annotated[str, Field(min_length=1, max_length=80, pattern=r"^[a-z][a-z0-9_-]*$")]
    selector: Annotated[str, Field(min_length=2, max_length=240, description="Class-based selector inside the target page, e.g. .ymb-about-hero__title or .ymb-advantage:nth-child(even).")]
    declarations: dict[str, str]
    min_width: Annotated[Optional[int], Field(ge=0, le=4000)] = None
    max_width: Annotated[Optional[int], Field(ge=0, le=4000)] = None


_ALLOWED_MANAGED_CSS_PROPERTIES = {
    ...
    'font-family','font-size','font-style','font-weight','line-height','letter-spacing','text-align','text-transform','white-space',
    ...
    'box-shadow','text-shadow','content','visibility','pointer-events',
}


def _validate_managed_style_rule(rule: ManagedStyleRule) -> dict[str, Any]:
    selector = rule.selector.strip()
    if not selector.startswith('.'):
        raise ValueError('Managed selector must start with a class selector.')
    if any(ch in selector for ch in '{};@#[]\\\r\n\"\''):
        raise ValueError('Managed selector contains disallowed characters.')
    lower_selector = selector.lower()
    for blocked in (':has(', ':not(', ':is(', ':where(', ':root', 'html', 'body'):
        if blocked in lower_selector:
            raise ValueError('Managed selector contains an unsupported scope or pseudo-class.')
    if rule.min_width is not None and rule.max_width is not None and rule.min_width > rule.max_width:
        raise ValueError('min_width cannot exceed max_width.')
    if not rule.declarations or len(rule.declarations) > 24:
        raise ValueError('Each managed rule must contain 1-24 declarations.')
    declarations: dict[str, str] = {}
    for prop, raw_value in rule.declarations.items():
        prop = str(prop).strip().lower()
        if prop not in _ALLOWED_MANAGED_CSS_PROPERTIES:
            raise ValueError(f'CSS property is not allow-listed: {prop}')
        value = str(raw_value).strip()
        if not value or len(value) > 220:
            raise ValueError(f'CSS value for {prop} is empty or too long.')
        if any(ch in value for ch in '{};@<>\\\r\n'):
            raise ValueError(f'CSS value for {prop} contains disallowed control characters.')
        lower_value = value.lower()
        for blocked in ('url(', 'expression(', 'javascript:', 'data:', '@import', '</style', '<script', '/*', '*/', '!important'):
            if blocked in lower_value:
                raise ValueError(f'CSS value for {prop} contains a disallowed construct.')
...
@mcp.tool(
    title="Read managed page styles",
    description=(
        "Read the current state hash and metadata for the Bridge-managed, page-scoped style control plane. "
        "READ-ONLY: this does not change WordPress files, database options, cache, or content. "
        "When bundle_id is supplied, the structured rules for that bundle are returned."
    ),
    annotations=ToolAnnotations(readOnlyHint=True, destructiveHint=False, idempotentHint=True, openWorldHint=False),
)
async def get_managed_style_state(bundle_id: str = "") -> dict[str, Any]:
    params = {"bundle_id": bundle_id} if bundle_id else None
    result = await _wp("GET", f"{NS}/design/styles", params=params)
    return result["data"]


@mcp.tool(
    title="Preview managed page style bundle",
    description=(
        "PREVIEW ONLY for routine visual/layout changes without reinstalling the WordPress Bridge. "
        "The caller supplies structured, page-scoped CSS rules only: a concrete WordPress page id, "
        "class-based selectors, allow-listed CSS properties, bounded breakpoint integers, and CSS values "
        "that cannot contain rule delimiters, URLs, script/data constructs, or stylesheet directives. "
        "The Bridge automatically prefixes every selector with body.page-id-<page_id>. No PHP, JavaScript, "
        "HTML, theme-file path, file content, search/replace payload, shell command, or global selector is accepted. "
        "This creates an immutable confirmation plan but does not activate the new styles."
    ),
    annotations=ToolAnnotations(readOnlyHint=False, destructiveHint=False, idempotentHint=False, openWorldHint=False),
)
async def preview_managed_style_bundle(
    bundle_id: Annotated[str, Field(min_length=1, max_length=64, pattern=r"^[a-z][a-z0-9_-]*$")],
    page_id: Annotated[int, Field(ge=1, description="Existing WordPress page id to which every rule is automatically scoped.")],
    expected_state_sha256: Annotated[str, Field(min_length=64, max_length=64)],
    rules: list[ManagedStyleRule],
    delete_bundle: bool = False,
) -> dict[str, Any]:
    if len(rules) > 100:
        raise ValueError('A managed style bundle may contain at most 100 rules.')
    safe_rules = [_validate_managed_style_rule(rule) for rule in rules]
    payload = {
        "bundle_id": bundle_id,
        "page_id": page_id,
        "expected_state_sha256": _validate_sha256_hex(expected_state_sha256),
        "rules": safe_rules,
        "delete_bundle": bool(delete_bundle),
    }
    result = await _wp("POST", f"{NS}/design/styles/preview", payload=payload)
    return result["data"]


@mcp.tool(
    title="Apply confirmed managed page styles",
    description=(
        "Apply exactly one previously previewed immutable managed-style plan. The caller cannot change selectors, "
        "properties, page scope, breakpoints, or values during apply. The Bridge re-checks the state hash, creates "
        "a durable managed-style rollback snapshot, writes the structured option, verifies its SHA-256, and returns "
        "the backup id. This does not edit theme PHP/CSS/JS files."
    ),
    annotations=ToolAnnotations(readOnlyHint=False, destructiveHint=True, idempotentHint=True, openWorldHint=False),
)
async def apply_managed_style_bundle(plan_id: str, confirmation_token: str) -> dict[str, Any]:
    result = await _wp("POST", f"{NS}/design/styles/apply", payload={"plan_id": plan_id, "confirmation_token": confirmation_token})
    return result["data"]


@mcp.tool(
    title="List managed style backups",
    description="READ-ONLY list of rollback snapshots created before managed-style applies/restores.",
    annotations=ToolAnnotations(readOnlyHint=True, destructiveHint=False, idempotentHint=True, openWorldHint=False),
)
async def list_managed_style_backups(limit: Annotated[int, Field(ge=1, le=100)] = 20) -> dict[str, Any]:
    result = await _wp("GET", f"{NS}/design/backups", params={"limit": limit})
    return result["data"]


@mcp.tool(
    title="Preview managed style rollback",
    description="PREVIEW ONLY a rollback to one managed-style backup. No active styles are changed until confirmed apply.",
    annotations=ToolAnnotations(readOnlyHint=False, destructiveHint=False, idempotentHint=False, openWorldHint=False),
)
async def preview_managed_style_restore(backup_id: str) -> dict[str, Any]:
    result = await _wp("POST", f"{NS}/design/restore/preview", payload={"backup_id": backup_id})
    return result["data"]


@mcp.tool(
    title="Apply confirmed managed style rollback",
    description="Apply exactly one previously previewed managed-style restore plan, with a fresh pre-restore safety snapshot.",
    annotations=ToolAnnotations(readOnlyHint=False, destructiveHint=True, idempotentHint=True, openWorldHint=False),
)
async def apply_managed_style_restore(plan_id: str, confirmation_token: str) -> dict[str, Any]:
    result = await _wp("POST", f"{NS}/design/restore/apply", payload={"plan_id": plan_id, "confirmation_token": confirmation_token})
    return result["data"]
