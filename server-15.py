...
_MARKETING_PLANS: dict[str, dict[str, Any]] = {}
...
def _prune_marketing_plans() -> None:
    now = time.time()
    expired = [
        plan_id for plan_id, plan in _MARKETING_PLANS.items()
        if float(plan.get("expires_at", 0)) <= now or bool(plan.get("used"))
    ]
    for plan_id in expired:
        _MARKETING_PLANS.pop(plan_id, None)
...
async def marketing_api_status() -> dict[str, Any]:
    return {
        ...
        "write_policy": "preview -> explicit user confirmation -> apply",
...
@mcp.tool(
    title="Preview Metrica write",
    description=(
        "PREVIEW ONLY. Prepare an immutable POST/PUT/DELETE request to an official "
        "Yandex Metrica API relative path. Does not send the write request. The resulting "
        "immutable plan can only be executed by apply_confirmed_metrika_write after explicit approval."
    ),
    annotations=ToolAnnotations(readOnlyHint=False, destructiveHint=False, idempotentHint=False, openWorldHint=True),
)
async def preview_metrika_write(
    method: str,
    path: str,
    params: Optional[dict[str, Any]] = None,
    payload: Optional[Any] = None,
...
@mcp.tool(
    title="Preview Yandex Direct write",
    description=(
        "PREVIEW ONLY. Prepare any non-get Direct API v5 service method as an immutable "
        "plan. The request is not sent until apply_confirmed_direct_write is called "
        "with this exact immutable plan and one-time confirmation token after explicit user approval."
    ),
    annotations=ToolAnnotations(readOnlyHint=False, destructiveHint=False, idempotentHint=False, openWorldHint=True),
)
async def preview_direct_write(
    service: str,
    method: str,
    params: dict[str, Any],
) -> dict[str, Any]:
...
async def _apply_confirmed_marketing_plan(
    ...
    *,
    ...
    """Internal executor. Never exposed as a generic MCP write tool."""
    _prune_marketing_plans()
    plan = _MARKETING_PLANS.get(plan_id)
    if not plan:
        raise ValueError("Marketing plan not found or expired.")
    ...
    if plan.get("used"):
        raise ValueError("Marketing plan has already been used.")
    if float(plan["expires_at"]) <= time.time():
        _MARKETING_PLANS.pop(plan_id, None)
        raise ValueError("Marketing plan has expired.")

    # Consume before the network call. We deliberately do not auto-retry unknown
    # transport outcomes: a blind retry of an add/update can duplicate a mutation.
    plan["used"] = True
    try:
...
@mcp.tool(
    title="Apply confirmed Yandex Direct write",
    description=(
        "WRITE. Apply exactly one immutable Yandex Direct plan that was already prepared by "
        "preview_direct_write and explicitly approved by the user. The caller can provide only "
        "the opaque plan id and its one-time confirmation token; it cannot choose or alter the "
        "Direct service, method, campaign, ad group, keyword, ad, budget, targeting, URL, or payload "
        "at apply time. Direct plans only; Metrica plans are rejected."
    ),
    annotations=ToolAnnotations(readOnlyHint=False, destructiveHint=True, idempotentHint=False, openWorldHint=True),
)
async def apply_confirmed_direct_write(
...
@mcp.tool(
    title="Apply confirmed Yandex Metrica write",
    description=(
        "WRITE. Apply exactly one immutable Yandex Metrica plan that was already prepared by "
        "preview_metrika_write and explicitly approved by the user. The caller can provide only "
        "the opaque plan id and its one-time confirmation token; it cannot choose or alter the "
        "Metrica path, HTTP method, counter, goal, parameters, upload content, or payload at apply "
        "time. Metrica plans only; Direct plans are rejected."
    ),
    annotations=ToolAnnotations(readOnlyHint=False, destructiveHint=True, idempotentHint=False, openWorldHint=True),
)
async def apply_confirmed_metrika_write(
